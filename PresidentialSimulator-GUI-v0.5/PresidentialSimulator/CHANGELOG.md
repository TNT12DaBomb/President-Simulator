...
# v0.4 Office & Interaction Edition

- Clear the map surface on every repaint; remove retained hover/resize artifacts.
- Use blue/red team colors and retain gold selection.
- Shuffle desktop debate answer positions deterministically and expand response prose.
- Add Cabinet, Public requests and Midterms under Office.
- Add human-readable policy previews and selectors; wrap long answers and help.
- Add pixel-based repaint and end-to-end Office regression tests.

---

# v0.3 Laptop Map Edition

Replaced the expanding state list with an offline geographic map and fixed inspector. Added immediate notifications, automatic decision pages, compact top stats and Menu hierarchy. Startup respects the usable desktop area, including taskbars. Added seed explanation and randomize control. Simulation classes are unchanged from v0.2.
