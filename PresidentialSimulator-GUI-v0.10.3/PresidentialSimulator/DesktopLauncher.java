...
public final class DesktopLauncher {
    ...
    public static void main(String[] args){
        ...
        SwingUtilities.invokeLater(()->{
            UIManager.put("OptionPane.background",DeskMenuFrame.PAPER);UIManager.put("Panel.background",DeskMenuFrame.PAPER);UIManager.put("OptionPane.messageForeground",DeskMenuFrame.INK);
            UIManager.put("Button.font",new Font(Font.SANS_SERIF,Font.PLAIN,14));
            DesktopController controller=new DesktopController();controller.autosaveTo(java.nio.file.Path.of(System.getProperty("user.home"),"PresidentialSimulator","saves","autosave.properties"));DesktopPanel panel=new DesktopPanel(controller);JFrame window=new JFrame("Presidential Simulator • v0.10.3");window.setDefaultCloseOperation(JFrame.DO_NOTHING_ON_CLOSE);window.setContentPane(panel);window.getRootPane().setDefaultButton(null);
            ...
            window.addWindowListener(new java.awt.event.WindowAdapter(){public void windowClosed(java.awt.event.WindowEvent e){if(!window.isDisplayable())timer.stop();}});window.setExtendedState(JFrame.MAXIMIZED_BOTH);window.setVisible(true);timer.start();
        });
    }
}
