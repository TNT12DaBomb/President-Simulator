...
public final class DesktopController {
    ...
    public int campaignColor(){return engine==null?0:engine.campaignColor();}
    public long careerSeed(){return engine==null?0:engine.seed();}
    public CareerView view() { return engine==null?null:engine.view(); }
    public DebateSettings debateSettings(){return engine==null?DebateSettings.DEFAULT:engine.debateSettings();}
    public List<String> debateCommitments(){return engine==null?List.of():engine.debateCommitments();}
    public List<LiveDebate.Statement> debateStatements(){return engine==null?List.of():engine.debateStatements();}
    ...
    public void start(long seed, President.Difficulty difficulty, President.RunningMate mate) {
        if(busy)return;
        engine=new CareerEngine(seed,difficulty,mate); messages=List.of("Your campaign begins. Your team colors are shown above the map. Visit Map to choose where to work. Each campaign action uses one turn.");
        last=clock.getAsLong();changed.run();
    }
    public void start(long seed,President.Difficulty difficulty,President.RunningMate mate,DebateSettings settings){start(seed,difficulty,mate);applyDebateSettings(settings);}
    public void applyDebateSettings(DebateSettings settings){
        submit(CareerCommand.office(OfficeCommand.choice(OfficeCommand.Type.DEBATE_MODE,settings.mode().ordinal())));
        submit(CareerCommand.office(OfficeCommand.choice(OfficeCommand.Type.DEBATE_PACE,settings.pace().ordinal())));
        submit(CareerCommand.office(OfficeCommand.choice(OfficeCommand.Type.DEBATE_TEXT,settings.textSize())));
        submit(CareerCommand.office(OfficeCommand.choice(OfficeCommand.Type.DEBATE_CELEBRATIONS,settings.celebrations()?1:0)));
    }
    public boolean submit(CareerCommand command) {
        ...
        else if(!oldDecision.equals(decisionKey())&&!decisionKey().isEmpty()){String title=view().phase()==CareerView.Phase.CAMPAIGN?view().campaign().pendingEvent().title():view().world().pending().title();publishNotice(title.startsWith("CORRECT")?"Correct answer":title.startsWith("INCORRECT")?"Answer correction":"Decision required",title,!title.startsWith("CORRECT"));}
