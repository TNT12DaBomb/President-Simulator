...
public class NewsUITests {
 ...
 public static void main(String[] args)throws Exception{SwingUtilities.invokeAndWait(()->{try{var c=new DesktopController(()->0L);var p=new DesktopPanel(c);c.start(44,President.Difficulty.NORMAL,President.RunningMate.FUNDRAISER);p.notice(new DesktopController.Notice("Test activity","This report remains until dismissal.",false));p.setSize(1024,650);layout(p);var n=label(p,"News headline");if(n==null||label(p,"$1,600")==null)throw new AssertionError("news and currency");int y=SwingUtilities.convertPoint(n,0,0,p).y;p.render();layout(p);if(y!=SwingUtilities.convertPoint(label(p,"News headline"),0,0,p).y)throw new AssertionError("news movement");var img=new BufferedImage(1024,650,BufferedImage.TYPE_INT_RGB);var g=img.createGraphics();p.printAll(g);g.dispose();ImageIO.write(img,"png",new File(System.getProperty("java.io.tmpdir"),"presidential-simulator-news-qa.png"));System.out.println("PASS: currency labels, fixed headline position and laptop render.");}catch(Exception e){throw new RuntimeException(e);}});}
}
