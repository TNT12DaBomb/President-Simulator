...
public final class CareerEngine {
    ...
    public CareerReport submit(CareerCommand command) {
        ...
        } else if (phase == CareerView.Phase.RETIRED && !(command.type()==CareerCommand.Type.OFFICE&&(command.office().type().name().startsWith("DEBATE_")||command.office().type()==OfficeCommand.Type.REALTIME_MODE||command.office().type()==OfficeCommand.Type.EVENT_FREQUENCY))) return reject("This career has ended. Start a new career or use a developer scenario.");
        int start = history.size();
        switch (command.type()) {
            case OFFICE -> {
                OfficeCommand o = command.office();
                if(o.type()==OfficeCommand.Type.REALTIME_MODE){if(!rules.timed())return reject("Live clocks require DECISIONS rules.");realtimeEnabled=o.choice()==1;history.add("Real-time response clocks "+(realtimeEnabled?"enabled":"paused; in-game deadlines still apply")+".");}
    ...
    private void beginTerm() {
        ...
        history.add("INAUGURATION: term " + wins + " begins. 48 monthly turns, two actions each month; end the month explicitly. Midterms follow month " + rules.midtermElectionMonth() + ".");
    }
