...
public final class TextUI {
    ...
    private void setup() {
        ...
        career = new CareerEngine(seed, President.Difficulty.values()[difficulty - 1], President.RunningMate.values()[mate - 1], GameRules.CURRENT);
    ...
    private void presidencyStep(CareerView v) {
        ...
        if (p.actionsLeft() == 0) text("This month's actions are used. Review records or choose End month to continue.");
        ...
        try { while(true){
            ...
            switch(topic){
                ...
                case 2 -> {
                    ...
                    text("Debates arrive on turns 4, 8 and 12. Each has three questions. Every question starts a fresh 60-second live countdown. Select an answer, then confirm it before time runs out.");
                    text("An unanswered question is scored as missed and the next question follows. Leaving skips all remaining questions. Details show costs and scoring; ST, then 7 shows your completed debate record.");
                ...
                case 3 -> {
                    text("Each presidential turn is one month. Choose up to two actions, inspect your situation, then select End month to move time forward.");separator();
    ...
    private void compactStats(){
        ...
        if(c==7){title("CAMPAIGN DEBATE RECORD");var game=career.view().campaign();
            if(game==null||game.debateResults().isEmpty())text("No completed multi-question debates in this campaign.");
