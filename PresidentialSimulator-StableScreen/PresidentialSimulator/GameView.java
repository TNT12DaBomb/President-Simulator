...
public record GameView(long seed, int turnsUsed, int totalTurns, int playerFunds, int opponentFunds,
                       ...
                       int fundraisingAmount, List<DebateSession.Result> debateResults, double debateShare) {
    ...
    public record PendingEvent(String id, String title, String description, String state, List<ChoiceView> choices, int dueTurn, int secondsRemaining, String missedResponse) {
        public PendingEvent(String id,String title,String description,String state,List<ChoiceView> choices){this(id,title,description,state,choices,0,0,"");}
        public PendingEvent { choices = List.copyOf(choices); }
    }
    public GameView {
        debateResults=List.copyOf(debateResults);
