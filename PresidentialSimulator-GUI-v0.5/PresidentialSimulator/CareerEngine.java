...
public final class CareerEngine {
    ...
    public CareerEngine(long seed, President.Difficulty difficulty, President.RunningMate mate, GameRules rules) {
        ...
        campaign = new GameEngine(seed, difficulty, mate, CampaignOpening.fresh(), rules.world(), 0, rules.timed(), rules.debates());
            ...
            case OFFICE -> {
                ...
                if(o.type()==OfficeCommand.Type.REALTIME_MODE){if(!rules.timed())return reject("Live clocks require DECISIONS rules.");realtimeEnabled=o.choice()==1;history.add("Real-time response clocks "+(realtimeEnabled?"enabled":"paused; in-game deadlines still apply")+".");}
                    ...
                    try {
                        ...
                        else { history.addAll(presidency.office(o)); if(o.type().name().startsWith("DEV_")) { developerUsed=true; history.add("DEVELOPER: "+o.type()+" / "+o.choice()); } syncOffice(); }
    ...
    private void startNextCampaign() {
        ...
        campaign.inheritDebateStatements(previousStatements);
    ...
    private void scenario(int term, int month) {
        ...
        campaign = new GameEngine(seed + (cycle - 1L) * 104729L, difficulty, mate, CampaignOpening.fresh(), rules.world(), 0, rules.timed(), rules.debates());
