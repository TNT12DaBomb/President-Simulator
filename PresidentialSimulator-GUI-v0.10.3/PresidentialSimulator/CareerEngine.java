...
public final class CareerEngine {
    ...
    public int campaignColor(){return campaignColor;}
    private DebateSettings debateSettings=DebateSettings.DEFAULT;
    public DebateSettings debateSettings(){return debateSettings;}
    public List<LiveDebate.Statement> debateStatements(){return campaign.debateStatements();}
    public List<String> debateCommitments(){return campaign.debateStatements().stream().filter(s->LiveDebate.commitment(s).isPresent()).map(s->s.debate()+": "+s.words()).toList();}
    private void recordPolicyComparison(OfficeCommand.Type action,Policy.Bill bill){if(bill==null)return;for(var s:campaign.debateStatements()){var commitment=LiveDebate.commitment(s);if(commitment.isPresent()&&commitment.get().issue()==bill.issue())history.add("DEBATE COMMITMENT REVIEW: You said “"+s.words()+"”. Now: "+action+" / "+bill.approach()+". "+(action==OfficeCommand.Type.SIGN&&commitment.get().approach()==bill.approach()?"The signed approach matches the commitment.":"This decision remains alongside the original commitment in your public record."));}}
    ...
    public CareerEngine(long seed, President.Difficulty difficulty, President.RunningMate mate) {
        this(seed, difficulty, mate, GameRules.CURRENT);
    }
    public CareerEngine(long seed, President.Difficulty difficulty, President.RunningMate mate, GameRules rules) {
        this.rules = Objects.requireNonNull(rules);realtimeEnabled=rules.timed();
        this.seed = seed; this.difficulty = Objects.requireNonNull(difficulty); this.mate = Objects.requireNonNull(mate);
        if (rules.world()) world = new WorldSimulation(seed,rules.timed());
        campaign = new GameEngine(seed, difficulty, mate, CampaignOpening.fresh(), rules.world(), 0, rules.timed(), rules.debates());campaign.debateSettings(debateSettings);
    ...
    public CareerReport submit(CareerCommand command) {
        ...
        } else if (phase == CareerView.Phase.RETIRED && !(command.type()==CareerCommand.Type.OFFICE&&(command.office().type().name().startsWith("DEBATE_")||command.office().type()==OfficeCommand.Type.REALTIME_MODE||command.office().type()==OfficeCommand.Type.EVENT_FREQUENCY||command.office().type()==OfficeCommand.Type.CAMPAIGN_COLOR))) return reject("This career has ended. Start a new career or use a developer scenario.");
        int start = history.size();
        switch (command.type()) {
            case OFFICE -> {
                OfficeCommand o = command.office();
                if(o.type()==OfficeCommand.Type.CAMPAIGN_COLOR){campaignColor=o.choice();history.add("Campaign color updated.");}
                else if(o.type()==OfficeCommand.Type.REALTIME_MODE){if(!rules.timed())return reject("Live clocks require DECISIONS rules.");realtimeEnabled=o.choice()==1;history.add("Real-time response clocks "+(realtimeEnabled?"enabled":"paused; in-game deadlines still apply")+".");}
                else if(o.type().name().startsWith("DEBATE_")){
                    debateSettings=switch(o.type()){
                        case DEBATE_MODE->new DebateSettings(DebateSettings.Mode.values()[o.choice()],debateSettings.pace(),debateSettings.textSize(),debateSettings.celebrations());
                        case DEBATE_PACE->new DebateSettings(debateSettings.mode(),DebateSettings.Pace.values()[o.choice()],debateSettings.textSize(),debateSettings.celebrations());
                        case DEBATE_TEXT->new DebateSettings(debateSettings.mode(),debateSettings.pace(),o.choice(),debateSettings.celebrations());
                        case DEBATE_CELEBRATIONS->new DebateSettings(debateSettings.mode(),debateSettings.pace(),debateSettings.textSize(),o.choice()==1);
                        default->throw new IllegalStateException();};
                    campaign.debateSettings(debateSettings);history.add("Debate settings saved. Mode and pace apply to the next appearance; text and feedback style update immediately.");
                }
                else if (o.type() == OfficeCommand.Type.PLEDGE) {
                    if (phase != CareerView.Phase.CAMPAIGN) return reject("Pledges are set during an active campaign.");
                    ...
                    eventFrequency = o.choice(); history.add("Presidency event frequency: " + (rules.world() ? new String[]{"Off", "Monthly 35% baseline", "Monthly 60% baseline"} : new String[]{"Off", "Every six months", "Every three months"})[eventFrequency] + ". Pending events are retained.");
                } else {
                    if (phase != CareerView.Phase.PRESIDENCY) return reject("This command requires an active presidency.");
                    if(o.type()==OfficeCommand.Type.CLOCK_TICK && !realtimeEnabled)return reject("Response clocks are paused.");
                    try {
                        if (o.type() == OfficeCommand.Type.END_MONTH) endMonth();
                        else { var beforeBill=presidency.view().bill();history.addAll(presidency.office(o));if(o.type()==OfficeCommand.Type.PROPOSE)recordPolicyComparison(o.type(),presidency.view().bill());else if(o.type()==OfficeCommand.Type.SIGN||o.type()==OfficeCommand.Type.VETO)recordPolicyComparison(o.type(),beforeBill); if(o.type().name().startsWith("DEV_")) { developerUsed=true; history.add("DEVELOPER: "+o.type()+" / "+o.choice()); } syncOffice(); }
                    } catch (IllegalArgumentException ex) { return reject(ex.getMessage()); }
                }
            }
            case CAMPAIGN -> {
                if(command.campaign().type()==GameCommand.Type.CLOCK_TICK && !realtimeEnabled)return reject("Response clocks are paused.");
        ...
        if (phase == CareerView.Phase.PRESIDENCY && presidency.view().actionsLeft() == 0 && presidency.view().pendingEvent() == null) endMonth();
        journal.add(command);
    ...
    private void recordElection(ElectionResult result, Boolean forcedWin) {
        ...
        if (world != null && result != null && !result.getVotes().isEmpty()) {
    ...
    private void beginTerm() {
        ...
        presidency = new Presidency(seed + cycle * 7919L, 2028 + year, pledges, transitionCredits, publicFeedback, rules, world);
        if (world != null) world.beginTerm(history);
        syncOffice();
        if (rules.current()) history.add("Inauguration: " + GameCalendar.inauguration(2028 + year) + ". First monthly turn includes the partial January; final January handoff remains compressed into term review.");
        history.add("INAUGURATION: term " + wins + " begins. 48 monthly turns, two actions each month; the month advances after your second action. Midterms follow month " + rules.midtermElectionMonth() + ".");
    }
    ...
    private void endMonth() {
        ...
        if (termMonths == MONTHS_PER_TERM) {
            ...
            presidency.view().cabinet().seats().stream().filter(s -> s.official() != null).forEach(s -> history.add("Archived cabinet seat: " + s.department() + " / " + (s.official() == null ? "Vacant" : s.official().name())));
    ...
    private void rebuild(CareerCommand.RebuildAction action) {
        ...
        if (world != null) {
            WorldEffect recovery = switch (action) {
                case CAMPAIGN_REVIEW -> WorldEffect.of(WorldMetric.REPUTATION,4,WorldMetric.TRUST,2);
                case DONOR_MEETINGS -> WorldEffect.of(WorldMetric.DONORS,7);
                case COMMUNITY_WORK -> WorldEffect.of(WorldMetric.POPULARITY,3,WorldMetric.REPUTATION,3);
                case PRIVATE_LIFE -> WorldEffect.of(WorldMetric.ENERGY,15);
            };
            world.apply(recovery, "Opposition rebuilding: " + action, history);
            for (int i=0;i<12;i++) history.addAll(world.advance(false));
        }
        comebackYears++; year++;
    ...
    private List<String> nextCampaignEffects() {
        ...
        if (world != null) effects.add(String.format(java.util.Locale.ROOT,"Next race modeled vote-share shift: %+.2f percentage points from approval, growth, jobs, favorability, reputation and ethics exposure.",world.electionMood()));
        if (world != null) effects.add("Reputation, donor support and job approval produce a private campaign cash adjustment of $" + world.campaignCash() + ". Public funds remain separate.");
    ...
    private void startNextCampaign() {
        int adjustment = (donorReviewDue ? -200 : 0) + (budgetPublished ? 100 : 0) + (world == null ? 0 : world.campaignCash());
        cycle++;
        ...
        var previousStatements=campaign.debateStatements();
        campaign = new GameEngine(nextSeed, difficulty, mate, new CampaignOpening(adjustment, work, reasons), rules.world(), world == null ? 0 : world.electionMood(), rules.timed(), rules.debates());
        campaign.inheritDebateStatements(previousStatements);campaign.debateSettings(debateSettings);
    ...
    private void scenario(int term, int month) {
        ...
        campaign = new GameEngine(seed + (cycle - 1L) * 104729L, difficulty, mate, CampaignOpening.fresh(), rules.world(), 0, rules.timed(), rules.debates());campaign.debateSettings(debateSettings);
    ...
    public CareerView view() {
        ...
        for (CareerCommand.GovernanceAction action : CareerCommand.GovernanceAction.values()) {
            ...
            if(reason.isEmpty() && world != null) reason=world.actionProblem(action);
        ...
        return new CareerView(phase, year, cycle, wins, losses, completedTerms, termMonths, servedMonths, comebackYears,
            ...
            pledges.entrySet().stream().map(e -> new Policy.Promise(e.getKey(), e.getValue(), "Campaign pledge")).toList(), rules, world == null ? null : world.view());
    }
}
