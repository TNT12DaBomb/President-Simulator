...
public record CareerCommand(Type type, GameCommand campaign, GovernanceAction governance,
                            RebuildAction rebuild, DeveloperAction developer, OfficeCommand office) {
    public enum Type { OFFICE, CAMPAIGN, CONTINUE, GOVERN, REBUILD, RUN_AGAIN, RETIRE, DEVELOPER }
    public enum GovernanceAction {
        ...
        APPOINT_OFFICIAL("Appoint an administrative official", 50), MANAGE_CRISIS("Coordinate a preparedness review", 75),
        CAMPAIGN_ALLIES("Complete an ally campaign commitment", 50), FUNDRAISE("Meet private donors", 0),
        TRAVEL("Visit a public service office", 50), REST("Reserve personal time", 0);
    ...
    public enum DeveloperAction {
        ...
        FINAL_QUARTER("Replace timeline: start the final month of term two"),
        ADD_CAMPAIGN_CASH("Add $500 campaign cash"), ADD_TREASURY("Add $500 to the treasury"),
        FINISH_TERM("Advance to the end of this term; decline outstanding events"),
        ADVANCE_MONTH("Advance one month"), CHANGE_CONGRESS("Toggle chamber control"), TRIGGER_EVENT("Trigger an eligible presidency event");
        private final String label;
        DeveloperAction(String label) { this.label = label; }
        @Override public String toString() { return label; }
    }
    public CareerCommand(Type type, GameCommand campaign, GovernanceAction governance, RebuildAction rebuild, DeveloperAction developer) {
        this(type, campaign, governance, rebuild, developer, null);
    }
    public static CareerCommand office(OfficeCommand command) { return new CareerCommand(Type.OFFICE, null, null, null, null, command); }
