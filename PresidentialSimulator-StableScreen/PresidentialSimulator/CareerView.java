...
public record CareerView(Phase phase, int year, int cycle, int electionsWon, int electionsLost,
                         int completedTerms, int termMonths, int servedMonths, int comebackYears,
                         int treasury, int monthlyReceipts, int routineExpenses, String publicFeedback,
                         ...
                         List<String> history, GameView campaign, boolean developerUsed, Presidency.View presidency, int eventFrequency, List<Policy.Promise> campaignPromises, GameRules rules, WorldSimulation.View world) {
    ...
    public CareerView {
        ...
        elections = List.copyOf(elections); history = List.copyOf(history); campaignPromises = List.copyOf(campaignPromises);
    }
}
