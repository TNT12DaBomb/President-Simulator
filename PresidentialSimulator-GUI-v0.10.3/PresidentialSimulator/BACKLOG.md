...
Nonblocking pending-decision UI; live debate window no longer modal; visible phone decision badge; map news; paper/debate palette consistency; expired stat-pulse replay fix.

## v0.10.2 completed

Consistent debate palette, themed stat fades, removal of remaining dropdown selectors, start-screen cleanup, roomier bottom navigation and wall/desk placement refinements.

## v0.10.1 completed

Campaign wall/desk layout; printed-paper map and searchable index; aligned bottom navigation; restrained pre-game UI; contextual report tabs; paged records; fuller source-record newspaper pages; clean keyboard focus treatment.

Next: native screen-reader/fullscreen testing, persist document page position across unrelated refreshes, richer event-authored article text, optional asset themes.

## v0.10.0 completed

Physical desk grouping; paper selections; document-local return; neutral dock with news first; grouped secondary systems; native fullscreen option; automatic two-action months; explicit save-version boundary.

## Next validation and UI tasks

- Native Windows/fullscreen/high-DPI and accessibility checks.
- Searchable state directory to replace the remaining 51-state dropdown.
- More compact paginated settings forms.
- Optional desk artwork/cosmetic packs using the existing asset interface.
- Migrate older test assumptions about manually ending each month.

## v0.9.3 completed

Unified bottom dock and menus; designed welcome screen; distinct object-themed workspace frames and matching controls. Native high-DPI and accessibility checks remain next UI validation tasks.

## v0.9.2 completed

Full-bleed desk shell, compact bottom HUD, pause-menu help, maximized startup, responsive shell checks.

Next UI checks: native laptop/high-DPI testing, richer replaceable desk art, accessible screen-reader audit, adaptive full-headline presentation.

# v0.9.1 correction pass

Completed: fixed menu placement, consistent paper/ink styles, bounded setup/settings forms, Escape navigation, fewer repeated headings and improved folder identity. Next: native Windows DPI/dialog testing, further workspace density review and replacement artwork without changing item targets.

# v0.9 completed

Two phase desks; no sidebar; all existing systems reachable; independent primary destinations; configurable asset/layout/visibility/notification/theme fields; replaceable artwork renderer; integrated headless navigation tests.

Next: native high-DPI and accessibility verification, migration/removal of old UI adapters/tests, validated external theme configuration, richer art packs. Staff/research/executive-order/intelligence systems remain separate future gameplay work.

# v0.8 desk milestone

Delivered: twelve interactive desk objects; original scalable artwork; mouse/keyboard controls; live decision indicator/countdown; procedural stack changes; return-to-desk navigation.

Next: integrated native laptop testing; screen-reader testing; more distinct folder art and subtle signing animation; executive-order and intelligence systems before adding dedicated clickable folders; relationships before contact-specific phone menus; budget framework before a functional budget binder. Coffee and clutter remain decorative until meaningful status inputs are designed.

# v0.7.4 UI milestone

Delivered: President’s Desk, month guidance, administration/legislation/delivery/request cards, on-map fundraising, Office phase gate and resource-change feedback.

Next UI work: native laptop testing of the new board and modal focus; richer project timelines and completion receipts; keyboard navigation across task cards. This update reuses existing simulation mechanics.

# v0.7.3 completed

Compact news, laptop navigation, saved selectable team palette and distinct opponent colors. Remaining: native high-DPI and screen-reader testing; deeper policy pages and long archives still scroll as needed.

# v0.7.2 completed

- Secure random seeds for new careers; seed controls removed from normal setup.
- Password-protected developer menu; one-use seed override and explicit reset.
- Full-width news panel, 48 ambient stories with deterministic per-turn rotation.
- Persistent Latest Activity inbox with individual dismissal; no expiring reports.

Next: topic-aware ambient headlines, a deeper news archive and outlet identities; smarter grouping of routine activity; native OS modal testing. Ambient stories currently add atmosphere without changing metrics.
