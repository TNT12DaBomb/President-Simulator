...
public final class DesktopPanel extends JPanel {
    ...
    public DesktopPanel(DesktopController controller) {
        this.controller=controller;debateWindow=new DebateWindow(this,controller,this::practice);setLayout(new BorderLayout(10,10));setBackground(BG);setBorder(new EmptyBorder(10,12,10,12));
    ...
    private void menu(){JPopupMenu menu=new JPopupMenu();String[] names={"Save / Load","Settings","Latest report","Developer","New career","Exit"};Runnable[] actions={this::saveMenu,this::settings,()->showReport(controller.messages),this::developer,this::newCareer,()->{Window w=SwingUtilities.getWindowAncestor(this);if(w!=null)requestClose(w::dispose);}};for(int i=0;i<names.length;i++){JMenuItem item=new JMenuItem(names[i]);Runnable action=actions[i];item.addActionListener(e->action.run());menu.add(item);}controller.pause();menu.addPopupMenuListener(new javax.swing.event.PopupMenuListener(){public void popupMenuWillBecomeVisible(javax.swing.event.PopupMenuEvent e){}public void popupMenuWillBecomeInvisible(javax.swing.event.PopupMenuEvent e){controller.resume();}public void popupMenuCanceled(javax.swing.event.PopupMenuEvent e){}});menu.show(this,12,Math.min(420,getHeight()-200));}
    ...
    public void render(){
        ...
        heading.setText(v==null?"v0.7  /  DESKTOP":"YOU: BLUE  |  OPPONENT: RED  ·  Year "+v.year()+" / "+v.phase().name().replace('_',' ')+(v.developerUsed()?"  /  DEV":""));
        JPanel body=column();body.add(label(page,22,INK));body.add(Box.createVerticalStrut(8));
        if(v==null){body.add(text("Build a campaign. Earn a mandate. Lead the country.\n\nStart a career or load a save to continue."));body.add(button("Start a new career",this::newCareer));body.add(button("Open a save",this::saveMenu));body.add(text("YOUR CAMPAIGN IS BLUE. Your opponent is RED. Select a red state on Map to begin challenging the opponent there."));content.add(scroll(body));}
        ...
        else if(page.equals("Decisions")){if(debateWindow.active()){event.add(label("Your debate has its own window",22,INK));event.add(text("Closing the debate window pauses it. Resume the same question when ready."));event.add(button("Open / resume live debate",debateWindow::open));}else decision(v);if(event.getComponentCount()==0)event.add(text("No decision is waiting. Continue your campaign or advance a month in office."));content.add(scroll(event));}
    ...
    private void dashboard(JPanel p,CareerView v){
        ...
        switch(v.phase()){
            case CAMPAIGN->{p.add(label("YOU — BLUE CAMPAIGN",20,new Color(115,178,255)));p.add(text("Red states currently favor your opponent. Gold marks your selected state."));p.add(button("Open electoral map",()->{page="Map";render();}));action(p,"Fundraise (+"+v.campaign().fundraisingAmount()+" campaign funds)",CareerCommand.campaign(GameCommand.fundraise()));action(p,"Rest / advance campaign",CareerCommand.campaign(GameCommand.rest()));}
    ...
    private JComponent mapPage(CareerView v){
        ...
        JTextArea legend=text("Blue: your statewide lead · Red: opponent · Gold: selected\nMaine / Nebraska can split EV; see State details.");legend.setFont(new Font(Font.SANS_SERIF,Font.PLAIN,12));west.add(legend,BorderLayout.SOUTH);JLabel identity=label("YOU — BLUE   /   OPPONENT — RED",17,new Color(115,178,255));identity.setName("Campaign identity");west.add(identity,BorderLayout.NORTH);
    ...
    private JComponent policyPage(CareerView v){
        ...
        Runnable describe=()->{policyIssue=issue.getSelectedIndex();policyApproach=approach.getSelectedIndex();var o=PolicyCatalog.option((Policy.Issue)issue.getSelectedItem(),(Policy.Approach)approach.getSelectedItem());preview.setText(commitmentNotes((Policy.Issue)issue.getSelectedItem())+o.title()+"\nBenefit: "+o.benefit()+"\nTradeoff: "+o.tradeoff()+"\nSigning cost: "+o.cost()+" office funds · Delivery: "+o.deliveryMonths()+" months.");};issue.addActionListener(e->describe.run());approach.addActionListener(e->describe.run());describe.run();
    ...
    private void decision(CareerView v){
        String key=controller.decisionKey();if(key.isEmpty())return;event.add(label("CURRENT DECISION",11,ACCENT));timer.setFont(new Font(Font.SANS_SERIF,Font.BOLD,18));event.add(timer);
    ...
    void updateTimer(){debateWindow.updateClock();timer.setForeground(ACCENT);timer.setText(controller.seconds()>0?(!controller.realtime()?"CLOCK DISABLED  ·  ":controller.paused()?"PAUSED  ·  ":"RESPOND IN  ")+controller.seconds()+"s":turnDeadline());}
    ...
    private void help(){showText("How to play",java.util.List.of(
        "GET STARTED\nYou are BLUE; the opponent is RED. Open Map, select a state and review its available actions. Each campaign action uses one turn. Fundraise on Dashboard when cash is low.",
    ...
    private void newCareer(){if(controller.busy())return;dialog(()->{
        ...
        JComboBox<President.Difficulty>d=new JComboBox<>(President.Difficulty.values());JComboBox<President.RunningMate>m=new JComboBox<>(President.RunningMate.values());friendly(d);friendly(m);
        JComboBox<DebateSettings.Mode> mode=new JComboBox<>(DebateSettings.Mode.values());friendly(mode);mode.setSelectedItem(setupSettings.mode());
        JComboBox<DebateSettings.Pace> pace=new JComboBox<>(DebateSettings.Pace.values());friendly(pace);pace.setSelectedItem(setupSettings.pace());
        JTextField seed=new JTextField(Long.toString(new java.security.SecureRandom().nextLong()));JButton random=button("Randomize seed",()->seed.setText(Long.toString(new java.security.SecureRandom().nextLong())));
        Object[] fields={"YOU ARE BLUE. Your opponent is RED.","Difficulty",d,"Running mate",m,"Debate questions: Politics = economics/civics; Fun = trivia/pop culture",mode,"Pace: Standard 30s answers / 20s recovery; Relaxed doubles; Untimed",pace,"Seed: same setup and commands reproduce a run",seed,random};
        if(JOptionPane.showConfirmDialog(this,fields,"New career — choose your debate mode",JOptionPane.OK_CANCEL_OPTION)==JOptionPane.OK_OPTION)try{setupSettings=new DebateSettings((DebateSettings.Mode)mode.getSelectedItem(),(DebateSettings.Pace)pace.getSelectedItem(),setupSettings.textSize(),setupSettings.celebrations());page="Dashboard";controller.start(Long.parseLong(seed.getText().strip()),(President.Difficulty)d.getSelectedItem(),(President.RunningMate)m.getSelectedItem(),setupSettings);controller.submit(CareerCommand.office(OfficeCommand.choice(OfficeCommand.Type.REALTIME_MODE,setupRealtime?1:0)));controller.submit(CareerCommand.office(OfficeCommand.choice(OfficeCommand.Type.EVENT_FREQUENCY,setupFrequency)));}catch(NumberFormatException e){JOptionPane.showMessageDialog(this,"Please enter a whole-number seed.");}
    });}
    ...
    private void practice(DebateSettings.Mode requestedMode){if(controller.busy())return;dialog(()->{
        var base=controller.view()==null?setupSettings:controller.debateSettings();JCheckBox timed=new JCheckBox("Use timed responses during practice",false);JComboBox<DebateSettings.Mode> mode=new JComboBox<>(DebateSettings.Mode.values());friendly(mode);mode.setSelectedItem(requestedMode==null?base.mode():requestedMode);
        ...
        var options=new DebateSettings((DebateSettings.Mode)mode.getSelectedItem(),timed.isSelected()?base.pace()==DebateSettings.Pace.UNTIMED?DebateSettings.Pace.STANDARD:base.pace():DebateSettings.Pace.UNTIMED,base.textSize(),base.celebrations());
    ...
    private void settings(){if(controller.busy())return;dialog(()->{
        var current=controller.view()==null?setupSettings:controller.debateSettings();JComboBox<DebateSettings.Mode> mode=new JComboBox<>(DebateSettings.Mode.values());mode.setSelectedItem(current.mode());friendly(mode);JComboBox<DebateSettings.Pace> pace=new JComboBox<>(DebateSettings.Pace.values());pace.setSelectedItem(current.pace());friendly(pace);
        ...
        JComboBox<String> frequency=new JComboBox<>(new String[]{"Off","Normal","Frequent"});frequency.setSelectedIndex(controller.view()==null?setupFrequency:controller.view().eventFrequency());
        JButton rehearsal=button("Practice selected mode · fixed tutorial",()->practice((DebateSettings.Mode)mode.getSelectedItem()));
        Object[] controls={"DEBATE CONTENT",mode,rehearsal,"Politics: "+DebateFactChecks.all().size()+" questions · Fun: "+DebateFactChecks.fun().size()+" questions","Mode and pace changes apply to the next debate.","RESPONSE PACE",pace,"Standard: 30s answer / 20s recovery. Relaxed doubles these.",clocks,"READABILITY — debate font size",size,feedback,"PRESIDENCY EVENT FREQUENCY",frequency,"Pending events are kept. Settings are saved with your career."};
        if(JOptionPane.showConfirmDialog(this,controls,"Settings",JOptionPane.OK_CANCEL_OPTION)==JOptionPane.OK_OPTION){setupSettings=new DebateSettings((DebateSettings.Mode)mode.getSelectedItem(),(DebateSettings.Pace)pace.getSelectedItem(),(Integer)size.getValue(),feedback.isSelected());setupRealtime=clocks.isSelected();setupFrequency=frequency.getSelectedIndex();if(controller.view()!=null){controller.applyDebateSettings(setupSettings);controller.submit(CareerCommand.office(OfficeCommand.choice(OfficeCommand.Type.REALTIME_MODE,clocks.isSelected()?1:0)));controller.submit(CareerCommand.office(OfficeCommand.choice(OfficeCommand.Type.EVENT_FREQUENCY,frequency.getSelectedIndex())));}render();}
    });}
